...
class AvailabilityBlockAdmin(admin.ModelAdmin):
    ...
    """
    list_display = ('start_date', 'end_date', 'start_time',
                    'end_time', 'display_days')
...
def confirm_bookings(modeladmin, request, queryset):
    ...
    modeladmin.message_user(
        request, f"Approved {updated} booking(s).", level=messages.SUCCESS)
...
def cancel_bookings(modeladmin, request, queryset):
    ...
    modeladmin.message_user(
        request, f"Cancelled {updated} booking(s).", level=messages.WARNING)
